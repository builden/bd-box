...
const logger = createLogger('routes/taskmasters');
...
router.get('/detect-all', async (req, res) => {
  try {
    // Import getProjects from the project-service module
    const { getProjects } = await import('../project-service.ts');
...
router.get('/next/:projectName', async (req, res) => {
  try {
    ...
    try {
      ...
      logger.warn('Failed to execute task-master CLI:', { message: cliError.message });
      ...
      const tasksResponse = await fetch(
        `http://localhost:${process.env.PORT || 3001}/api/taskmaster/tasks/${encodeURIComponent(projectName)}`,
        {
          headers: {
...
router.post('/add-task/:projectName', async (req, res) => {
  try {
    ...
    addTaskProcess.on('close', (code) => {
      logger.info('Add task process completed with code:', { code, stdout, stderr });
