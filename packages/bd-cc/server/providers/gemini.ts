  ...
  handleEvent(event) {
    ...
    if (event.type === 'message' && event.role === 'assistant') {
      ...
      const payload: Record<string, unknown> = {
        ...
        data: { type: 'message', content, isPartial: event.delta === true },
      };
      if (socketSessionId) payload.sessionId = socketSessionId;
      this.ws.send(payload);
    } else if (event.type === 'tool_use') {
      if (this.onToolUse) this.onToolUse(event);
      const payload: Record<string, unknown> = {
      ...
      if (this.onToolResult) this.onToolResult(event);
      const payload: Record<string, unknown> = {
      ...
      const payload: Record<string, unknown> = {
        type: 'gemini-response',
        data: { type: 'message', content: '', isPartial: false },
      };
      if (socketSessionId) payload.sessionId = socketSessionId;
      this.ws.send(payload);
      if (event.stats?.total_tokens) {
        const statsPayload: Record<string, unknown> = {
          type: 'claude-status',
          data: { status: 'Complete', tokens: event.stats.total_tokens },
        };
  ...
  return new Promise((resolve, reject) => {
    ...
    geminiProcess.on('close', async (code) => {
      ...
      if (code === 0) {
        ...
        reject(
          new Error(
            code === null ? 'Gemini CLI process was terminated or timed out' : `Gemini CLI exited with code ${code}`
          )
        );
      }
    });
