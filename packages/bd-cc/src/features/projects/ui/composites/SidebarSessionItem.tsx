...
type SidebarSessionItemProps = {
  ...
  onDeleteSession: (projectName: string, sessionId: string, sessionTitle: string, provider: SessionProvider) => void;
  t: TFunction;
};
...
function SidebarSessionItem({
  ...
  }, [project, session, onProjectSelect, onSessionSelect]);

  const saveEditedSession = useCallback(() => {
    onSaveEditingSession(project.name, session.id, editingSessionName, session.__provider);
  }, [project.name, session.id, editingSessionName, session.__provider, onSaveEditingSession]);

  const requestDeleteSession = useCallback(() => {
    onDeleteSession(project.name, session.id, sessionView.sessionName, session.__provider);
  }, [project.name, session.id, sessionView.sessionName, session.__provider, onDeleteSession]);
