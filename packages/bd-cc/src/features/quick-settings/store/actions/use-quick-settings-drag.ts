...
export function useQuickSettingsDrag({ isMobile }: UseQuickSettingsDragProps) {
  const [handlePosition] = useAtom(handlePositionAtom);
  const handlePositionValue = handlePosition.y;
  const [, setHandlePosition] = useAtom(handlePositionAtom);
  ...
  const handleMove = useCallback(
    (event: MoveDragEvent) => {
      ...
      const newPosition = dragStartPositionRef.current + positionDelta;
      setHandlePosition({ y: Math.max(HANDLE_POSITION_MIN, Math.min(HANDLE_POSITION_MAX, newPosition)) });
    },
    [applyBodyDragStyles, isMobile, isPointerDown, setHandlePosition]
  );
  ...
  const startDrag = useCallback(
    (event: StartDragEvent) => {
      ...
      dragStartPositionRef.current = handlePositionValue;
      didDragRef.current = false;
      setIsDragging(false);
  ...
  const handleStyle = useMemo<QuickSettingsHandleStyle>(() => {
    if (!isMobile || typeof window === 'undefined') {
      return {
        top: `${handlePositionValue}%`,
        transform: 'translateY(-50%)',
      };
    }

    return {
      bottom: `${(window.innerHeight * handlePositionValue) / 100}px`,
    };
  }, [handlePositionValue, isMobile]);
