    ...
    return (
      <button
        ...
        className={cn(
          ...
          isSelected ? 'bg-background text-foreground shadow-sm' : 'text-muted-foreground active:bg-background/50'
        )}
      >
        <div className="flex items-center justify-center gap-1.5">
          ...
          {authStatus.authenticated && <span className={`h-1.5 w-1.5 flex-shrink-0 rounded-full ${colors.dot}`} />}
        </div>
      </button>
    );
  }
