...
export default function Onboarding({ onComplete }: OnboardingProps) {
  ...
  const handleProviderLoginOpen = async (provider: CliProvider) => {
    // Refresh auth status before opening login modal to get the latest state
    await checkProviderAuthStatus(provider);
    ...
    <>
      ...
      {activeLoginProvider && (
        <ProviderLoginModal
          ...
          isOnboarding={true}
          isAuthenticated={providerStatuses[activeLoginProvider]?.authenticated ?? false}
        />
      )}
    </>
  );
