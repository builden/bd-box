...
describe("API Endpoints", () => {
  ...
  describe("User API", () => {
    // In platform mode, protected routes may return 200 without auth
    it("GET /api/user/git-config should return 200", async () => {
      const res = await fetch(`${BASE_URL}/api/user/git-config`);
      expect(res.status).toBe(200);
      const data = await res.json();
      expect(data).toHaveProperty("success");
    });

    it("GET /api/user/onboarding-status should return 200", async () => {
