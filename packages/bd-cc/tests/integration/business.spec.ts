...
describe("Integration Tests - Business Logic", () => {
  ...
  describe("Projects Integration", () => {
    it("should return projects for claude provider", async () => {
      // Note: /api/projects/:provider may return HTML (SPA fallback)
      const res = await fetch(`${BASE_URL}/api/projects/claude`);
      // Check if we get 200 with JSON or HTML
      const contentType = res.headers.get("content-type") || "";
      if (contentType.includes("json")) {
        const data = await res.json();
        expect(data).toHaveProperty("name");
      } else {
        // HTML fallback is acceptable
  ...
  describe("Terminal WebSocket Integration", () => {
    it("should connect to WebSocket and receive welcome message", async () => {
