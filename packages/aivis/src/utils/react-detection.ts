...
function hasReactFiber(element: Element): boolean {
  return Object.keys(element).some(
    (key) =>
      key.startsWith('__reactFiber$') || key.startsWith('__reactInternalInstance$') || key.startsWith('__reactProps$')
  );
}
 ...
 */
export function getReactComponentName(element: HTMLElement, config?: ReactDetectionConfig): ReactComponentInfo {
