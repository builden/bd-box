...
let _domCaptureModule:
  ...
  | null
  | undefined; // null = tried and failed, undefined = not tried yet
 ...
 */
function findCaptureTarget(captureX: number, captureY: number, captureW: number, captureH: number): HTMLElement {
...
export async function captureDomRegion(
  ...
  try {
    ...
    ctx.drawImage(domCanvas, elContentX, elContentY, cropW, cropH, 0, 0, outW, outH);
