...
function HeroSkeleton({ width, height, text }: SkeletonProps) {
  return (
    ...
    >
      ...
      <Bar w={width * 0.4} />
      <Block
        w={Math.min(140, width * 0.2)}
        h={Math.min(36, height * 0.12)}
        radius={6}
...
function ModalSkeleton({ width, _height }: SkeletonProps) {
  return (
...
function CardSkeleton({ _width, _height }: SkeletonProps) {
  return (
    <div style={{ height: '100%', display: 'flex', flexDirection: 'column' }}>
...
function ListSkeleton({ _width, height }: SkeletonProps) {
...
function TabsSkeleton({ width, _height }: SkeletonProps) {
    ...
    <div style={{ height: '100%', display: 'flex', flexDirection: 'column' }}>
      <div style={{ display: 'flex', gap: 2, borderBottom: '1px solid var(--agd-stroke)' }}>
        {Array.from({ length: tabCount }, (_, i) => (
          <div
            key={i}
            style={{ padding: '8px 12px', borderBottom: i === 0 ? '2px solid var(--agd-bar-strong)' : 'none' }}
          >
...
function SectionSkeleton({ width, height }: SkeletonProps) {
  return (
    <div
      style={{ display: 'flex', flexDirection: 'column', height: '100%', gap: height * 0.04, padding: width * 0.04 }}
    >
      <Bar w={width * 0.3} h={4} strong />
      <Bar w={width * 0.7} />
      <Bar w={width * 0.5} />
    ...
    <div style={{ height: '100%', display: 'flex', flexDirection: 'column' }}>
      ...
      <div style={{ flex: 1, padding: 4, display: 'flex', flexDirection: 'column', gap: 2 }}>
        {Array.from({ length: items }, (_, i) => (
          <div
            key={i}
            style={{ padding: '4px 6px', borderRadius: 3, background: i === 0 ? 'var(--agd-fill)' : 'transparent' }}
          >
...
function SearchSkeleton({ _width, height }: SkeletonProps) {
...
function ToastSkeleton({ _width, height }: SkeletonProps) {
  return (
    <div
      style={{
        height: '100%',
...
function ChartSkeleton({ width, _height }: SkeletonProps) {
...
function VideoSkeleton({ width, height }: SkeletonProps) {
  ...
  return (
    <div
      style={{ height: '100%', position: 'relative', display: 'flex', alignItems: 'center', justifyContent: 'center' }}
    >
...
function TooltipSkeleton({ _width, _height }: SkeletonProps) {
  return (
...
function BreadcrumbSkeleton({ width, _height }: SkeletonProps) {
...
function DividerSkeleton({ _width }: SkeletonProps) {
  return (
...
function AccordionSkeleton({ _width, height }: SkeletonProps) {
...
function CarouselSkeleton({ _width, _height }: SkeletonProps) {
  return (
...
function TestimonialSkeleton({ _width, _height }: SkeletonProps) {
  return (
...
function CtaSkeleton({ width, height }: SkeletonProps) {
  return (
    ...
    >
      ...
      <Bar w={width * 0.35} />
      <Block
        w={Math.min(140, width * 0.25)}
        h={Math.min(32, height * 0.15)}
        radius={6}
        style={{ marginTop: height * 0.04, background: 'var(--agd-bar)' }}
      />
    </div>
  );
}

function AlertSkeleton({ _width, _height }: SkeletonProps) {
  return (
    <div
      style={{
        height: '100%',
...
function TagSkeleton({ width, _height }: SkeletonProps) {
  return (
    <div
      style={{
        height: '100%',
...
function TimelineSkeleton({ _width, height }: SkeletonProps) {
  ...
  return (
    <div style={{ display: 'flex', height: '100%', padding: '8px 0' }}>
      ...
      <div
        style={{ flex: 1, display: 'flex', flexDirection: 'column', justifyContent: 'space-around', paddingLeft: 8 }}
      >
        {Array.from({ length: items }, (_, i) => (
...
function CodeBlockSkeleton({ _width, height }: SkeletonProps) {
    ...
    <div style={{ height: '100%', display: 'flex', flexDirection: 'column' }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: `repeat(${cols}, 1fr)`, gap: 2, padding: '0 4px', flex: 1 }}>
        {Array.from({ length: cols }, (_, i) => (
          <div
            key={`h${i}`}
            style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', height: cellSz * 0.6 }}
          >
...
function NotificationSkeleton({ _width, height }: SkeletonProps) {
  return (
    <div
      style={{
        height: '100%',
...
function ProductCardSkeleton({ width, _height }: SkeletonProps) {
  return (
    <div style={{ height: '100%', display: 'flex', flexDirection: 'column' }}>
...
function PopoverSkeleton({ _width, _height }: SkeletonProps) {
  return (
    <div style={{ height: '100%', display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
      <div
        style={{
    ...
    <div style={{ height: '100%', display: 'flex', flexDirection: 'column', gap: 4 }}>
      ...
      >
        ...
        <svg width="12" height="12" viewBox="0 0 16 16" fill="none">
          <rect x="2" y="3" width="12" height="11" rx="1" stroke="var(--agd-stroke)" strokeWidth="1" />
          <line x1="2" y1="6" x2="14" y2="6" stroke="var(--agd-stroke)" strokeWidth="0.5" />
        </svg>
      </div>
      <div
        style={{
      ...
      >
        ...
        <div
          style={{ display: 'grid', gridTemplateColumns: `repeat(${cols}, 1fr)`, gap: 1, padding: '0 4px', flex: 1 }}
        >
          {Array.from({ length: cols * rows }, (_, i) => (
            <div key={i} style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', height: cellSz }}>
              <div
...
function SkeletonSkeletonRenderer({ _width, height }: SkeletonProps) {
  return (
    <div style={{ height: '100%', display: 'flex', flexDirection: 'column', gap: height * 0.08, padding: 4 }}>
      ...
      <div
        style={{ width: '70%', height: Math.max(6, height * 0.1), borderRadius: 3, background: 'var(--agd-fill)' }}
      />
      <div
        style={{ width: '90%', height: Math.max(4, height * 0.06), borderRadius: 3, background: 'var(--agd-fill)' }}
      />
      <div
        style={{ width: '50%', height: Math.max(4, height * 0.06), borderRadius: 3, background: 'var(--agd-fill)' }}
      />
    </div>
  );
}

function ChipSkeleton({ _width, height }: SkeletonProps) {
  return (
...
function FeatureSkeleton({ width, height }: SkeletonProps) {
  ...
  return (
    <div
      style={{ display: 'flex', flexDirection: 'column', height: '100%', justifyContent: 'space-around', padding: 8 }}
    >
      {Array.from({ length: items }, (_, i) => (
  ...
  return (
    ...
    >
      ...
      <Block
        ...
        h={Math.min(36, height * 0.12)}
        radius={6}
        style={{ marginTop: height * 0.03, background: 'var(--agd-bar)' }}
      />
      <Bar w={width * 0.4} h={2} />
    </div>
  );
...
function ContactSkeleton({ width, height }: SkeletonProps) {
  return (
    <div
      style={{ height: '100%', display: 'flex', flexDirection: 'column', padding: width * 0.04, gap: height * 0.03 }}
    >
      ...
      <Block
        w={Math.min(120, width * 0.3)}
        h={Math.min(30, height * 0.1)}
        radius={6}
