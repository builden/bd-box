...
export function RearrangeOverlay({
  ...
  const [exitingConnectors, setExitingConnectors] = useState<
    Map<
      string,
      {
  ...
  const lastChangedRectsRef = useRef<
    Map<
      string,
      {
  ...
  useEffect(() => {
    const handleMouseDown = (e: MouseEvent) => {
      ...
      if (target && !alreadyCaptured) {
        ...
        const _origRect = section.originalRect;
        let moved = false;
        let lastDx = 0,
          lastDy = 0;
          ...
          sections: sections.map((s) =>
            selectedIds.has(s.id)
              ? {
                  ...
                  currentRect: {
                    ...s.currentRect,
                    x: Math.max(0, s.currentRect.x + dx),
                    y: Math.max(0, s.currentRect.y + dy),
                  },
                }
              : s
          ),
    ...
    (e: React.MouseEvent, id: string) => {
      ...
      for (const s of sections) {
        if (newSelected.has(s.id)) {
          ...
          dragEls.set(s.id, {
            outlineEl,
            curW: s.currentRect.width,
            curH: s.currentRect.height,
          });
        }
      }
  ...
  for (const s of changedSections) {
    lastChangedRectsRef.current.set(s.id, {
          ...
          return (
            <div
              ...
              style={{
                ...
                ...(outlinesReady ? {} : { opacity: 0, animation: 'none', transition: 'none' }),
              }}
            ...
            >
              ...
              <span className={`${styles.sectionAnnotation} ${section.note ? styles.annotationVisible : ''}`}>
                {(() => {
              ...
              <span className={styles.ghostBadge}>
                {(() => {
                  ...
                  if (moved && resized) {
                    ...
                    return (
                      <>
                        Suggested {a} <span className={styles.ghostBadgeExtra}>&amp; {b}</span>
                      </>
                    );
                  }
      ...
      {!blankCanvas &&
        (() => {
          ...
          const connectorSections: {
            id: string;
            orig: (typeof sections)[0]['originalRect'];
            target: { x: number; y: number; width: number; height: number };
            isFixed?: boolean;
            isSelected: boolean;
            isExiting?: boolean;
