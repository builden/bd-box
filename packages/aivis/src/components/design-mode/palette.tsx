  ...
  switch (type) {
    ...
    case 'testimonial':
      return (
        <svg viewBox="0 0 20 16" width="20" height="16" fill="none">
          <rect x="2" y="1" width="16" height="14" rx="1.5" stroke={s} strokeWidth={sw} />
          <text x="4" y="5.5" fontSize="4" fill={s} opacity=".2" fontFamily="serif">
            &ldquo;
          </text>
    ...
    case 'faq':
      return (
        <svg viewBox="0 0 20 16" width="20" height="16" fill="none">
          <text x="2.5" y="5.5" fontSize="4" fill={s} opacity=".3" fontWeight="bold">
            ?
          </text>
          <rect x="7" y="3" width="10" height="1" rx=".5" fill={s} opacity=".25" />
          <rect x="7" y="5.5" width="8" height="1" rx=".5" fill={s} opacity=".12" />
          <text x="2.5" y="11.5" fontSize="4" fill={s} opacity=".3" fontWeight="bold">
            ?
          </text>
  ...
  if (suffixChanged) {
    ...
    return (
      <span className={styles.rollingWrap}>
        ...
        <span
          key={`o${prev}-${value}`}
          className={`${styles.rollingNum} ${dir === 'up' ? styles.exitUp : styles.exitDown}`}
        >
          {prev} {prevSuffix}
        </span>
        <span key={`n${value}`} className={`${styles.rollingNum} ${dir === 'up' ? styles.enterUp : styles.enterDown}`}>
          {value} {suffix}
        </span>
      </span>
    );
...
type DesignPaletteProps = {
  ...
  _Tooltip?: React.ComponentType<{ content: string; children: React.ReactNode }>;
};
...
export function DesignPalette({
  ...
  if (totalCount > 0) {
    ...
    lastFooterSuffix.current = blankCanvas
      ? totalCount === 1
        ? 'Component'
        : 'Components'
      : totalCount === 1
