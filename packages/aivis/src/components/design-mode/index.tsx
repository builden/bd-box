...
function computeSnap(
  ...
  const mL = rect.x,
    mR = rect.x + rect.width,
    mCx = rect.x + rect.width / 2;
  const mT = rect.y,
    mB = rect.y + rect.height,
    mCy = rect.y + rect.height / 2;

  const checkAll = !activeEdges;
  const xFroms = checkAll ? [mL, mR, mCx] : [...(activeEdges.left ? [mL] : []), ...(activeEdges.right ? [mR] : [])];
  const yFroms = checkAll ? [mT, mB, mCy] : [...(activeEdges.top ? [mT] : []), ...(activeEdges.bottom ? [mB] : [])];
...
export function DesignMode({
    ...
    (e: React.MouseEvent, id: string) => {
      ...
      const onMove = (ev: MouseEvent) => {
        ...
        const {
          ...
          dy: snapDy,
          guides: newGuides,
        } = computeSnap(selRect, basePlacements, new Set(startPositions.keys()), undefined, extraSnapRects);
        setGuides(newGuides);
    ...
    (e: React.MouseEvent, id: string, dir: HandleDir) => {
      ...
      const onMove = (ev: MouseEvent) => {
        ...
        const {
          ...
          dy: snapDy,
          guides: newGuides,
        } = computeSnap(rect, placementsRef.current, new Set([id]), activeEdges, extraSnapRects);
        setGuides(newGuides);
        ...
        onChange(placementsRef.current.map((p) => (p.id === id ? { ...p, x: nx, y: ny, width: nw, height: nh } : p)));
  ...
  const _TEXT_TYPES = new Set<ComponentType>([
    'text',
    'hero',
    'button',
    'badge',
  ...
  const edgeHandles: { dir: HandleDir; cls: string; arrow: JSX.Element }[] = [
    {
      ...
      arrow: (
        <svg width="8" height="6" viewBox="0 0 8 6" fill="none">
          <path d="M4 0.5L1 4.5h6z" fill={arrowColor} />
        </svg>
      ),
    },
    {
      ...
      arrow: (
        <svg width="6" height="8" viewBox="0 0 6 8" fill="none">
          <path d="M5.5 4L1.5 1v6z" fill={arrowColor} />
        </svg>
      ),
    },
    {
      ...
      arrow: (
        <svg width="8" height="6" viewBox="0 0 8 6" fill="none">
          <path d="M4 5.5L1 1.5h6z" fill={arrowColor} />
        </svg>
      ),
    },
    {
      ...
      arrow: (
        <svg width="6" height="8" viewBox="0 0 6 8" fill="none">
          <path d="M0.5 4L4.5 1v6z" fill={arrowColor} />
        </svg>
      ),
    },
  ];
        ...
        {placements.map((p) => {
          ...
          return (
            ...
            >
              ...
              <span className={`${styles.placementAnnotation} ${p.text ? styles.annotationVisible : ''}`}>
                {(() => {
