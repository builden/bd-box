...
import { AnnotationMarkerList } from './components/AnnotationMarkerList';
import { HoverTooltip } from './components/HoverTooltip';
import { HoverHighlight } from './components/HoverHighlight';
import { MarkerHoverOutline } from './components/MarkerHoverOutline';
import { BlankCanvasBackdrop } from './components/BlankCanvasBackdrop';
import { WireframeHint } from './components/WireframeHint';
import { MultiSelectHighlights } from './components/MultiSelectHighlights';
...
export function PageFeedbackToolbarCSS({
  ...
  const wireframeStashRef = useRef<{ rearrange: RearrangeState | null; placements: DesignPlacement[] }>({
    rearrange: null,
    placements: [],
  });
  ...
  useEffect(() => {
    ...
    if (blankCanvas) {
      ...
      const hasContent =
        (stash.rearrange?.sections?.length ?? 0) > 0 || stash.placements.length > 0 || wireframePurpose;
      if (hasContent) {
        saveWireframeState(pathname, {
  ...
  useEffect(() => {
    ...
    const handleClick = (e: MouseEvent) => {
      ...
      if (e.metaKey && e.shiftKey && !pendingAnnotation && !editingAnnotation) {
        ...
        const existingIndex = pendingMultiSelectElements.findIndex((item) => item.element === elementUnder);
        ...
        syncAnnotation(endpoint, currentSessionId, newAnnotation)
          .then((serverAnnotation) => {
            ...
            if (serverAnnotation.id !== newAnnotation.id) {
              setAnnotations((prev) =>
                prev.map((a) => (a.id === newAnnotation.id ? { ...a, id: serverAnnotation.id } : a))
              );
  ...
  const copyOutput = useCallback(async () => {
    ...
    if (!wireframeOnly && drawStrokes.length > 0) {
      ...
      for (let strokeIdx = 0; strokeIdx < drawStrokes.length; strokeIdx++) {
        ...
        const viewportPoints = stroke.fixed ? stroke.points : stroke.points.map((p) => ({ x: p.x, y: p.y - scrollY }));
            ...
            <div className={styles.buttonWrapper}>
              ...
              >
                <IconCopyAnimated
                  ...
                  copied={copied}
                  tint={
                    isDesignMode && blankCanvas && (designPlacements.length > 0 || !!rearrangeState?.sections?.length)
                      ? '#f97316'
                      : undefined
                  }
                />
              ...
              setRearrangeState((prev) => {
                ...
                return {
                  ...
                  sections: prev.sections.map((s) => {
                    ...
                    return {
                      ...s,
                      currentRect: { ...s.currentRect, x: Math.max(0, start.x + dx), y: Math.max(0, start.y + dy) },
                    };
                  }),
                };
              });
