...
function ModalSkeleton({ width }: SkeletonProps) {
  return (
...
function ListSkeleton({ height }: SkeletonProps) {
...
function TabsSkeleton({ width }: SkeletonProps) {
...
function SearchSkeleton({ height }: SkeletonProps) {
...
function ToastSkeleton({ height }: SkeletonProps) {
  return (
    <div
      style={{
        height: '100%',
...
function ChartSkeleton({ width }: SkeletonProps) {
...
function BreadcrumbSkeleton({ width }: SkeletonProps) {
  ...
  return (
    <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', height: '100%', gap: 4 }}>
      {Array.from({ length: count }, (_, i) => (
        <Block key={i} w={sz} h={sz} radius={4} {...(i === 1 ? { style: { background: 'var(--agd-bar)' } } : {})} />
      ))}
    </div>
  );
}
...
function AccordionSkeleton({ height }: SkeletonProps) {
...
function TagSkeleton({ width }: SkeletonProps) {
  return (
    <div
      style={{
        height: '100%',
...
function TimelineSkeleton({ height }: SkeletonProps) {
...
function CodeBlockSkeleton({ height }: SkeletonProps) {
...
function NotificationSkeleton({ height }: SkeletonProps) {
  return (
    <div
      style={{
        height: '100%',
...
function ProductCardSkeleton({ width }: SkeletonProps) {
  return (
    <div style={{ height: '100%', display: 'flex', flexDirection: 'column' }}>
...
function SkeletonSkeletonRenderer({ height }: SkeletonProps) {
  return (
...
function ChipSkeleton({ height }: SkeletonProps) {
  return (
...
export function Skeleton({
  ...
  return (
    <div style={{ width: '100%', height: '100%', padding: 8, position: 'relative', pointerEvents: 'none' }}>
      <Renderer width={width} height={height} {...(text !== undefined ? { text } : {})} />
    </div>
  );
}
