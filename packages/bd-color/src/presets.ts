...
/** 从颜色映射创建调色板对象 */
function createPalettes<T>(colorMap: Record<string, string>, createFn: (color: string) => T): Record<string, T> {
  return Object.fromEntries(Object.entries(colorMap).map(([name, color]) => [name, createFn(color)]));
}
