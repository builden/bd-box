...
describe('DiagramManager', () => {
  ...
  describe('config variations', () => {
    ...
    it('should not setup resize when resizable is false', () => {
      ...
      const handle = container.querySelector('.diagram-resize-handle');
  ...
  describe('zoom controls', () => {
    it('should create zoom in button', () => {
      ...
      expect(zoomInBtn?.querySelector('svg')).not.toBeNull();
    });
    ...
    it('should create zoom out button', () => {
      ...
      expect(zoomOutBtn?.querySelector('svg')).not.toBeNull();
    });
    ...
    it('should create copy and navigation controls', () => {
      ...
      expect(container.querySelector('#test-id-pan-down')).not.toBeNull();
      expect(container.querySelector('#test-id-reset')).not.toBeNull();
      expect(container.querySelector('#test-id-zoom-in')).not.toBeNull();
      expect(container.querySelector('#test-id-zoom-out')).not.toBeNull();
    });

    it('should reset to the origin view', () => {
      const { container } = createContainer();
      container.classList.add('mermaid');
      manager.setup('test-id', container);

      const svg = container.querySelector('svg') as SVGSVGElement;
      const target = svg.querySelector('g') as SVGGElement;
      target.style.transform = 'translate(50px, 50px) scale(2)';

      const resetBtn = container.querySelector('#test-id-reset') as HTMLButtonElement;
      resetBtn.click();
    ...
    it('should move container to body when fullscreen is toggled', () => {
      ...
      const fullscreenBtn = container.querySelector('#test-id-fullscreen') as HTMLButtonElement;
      fullscreenBtn.click();

      expect(container.classList.contains('fullscreen')).toBe(true);
      expect(container.parentElement).toBe(document.body);
      expect(document.body.classList.contains('diagram-fullscreen-active')).toBe(true);
